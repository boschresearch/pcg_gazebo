...
class HingedDoor(SimulationModel):
    ...
    def _set_door_model_origin_to_ground(self):
        if self._door_link_name not in self.link_names:
            PCG_ROOT_LOGGER.warning(
                '[{}] Door link does not exist, model origin will not be set'.format(
                    self._name))
    ...
    def _add_hinge_joint(self):
        if self._door_link_name not in self.link_names:
            PCG_ROOT_LOGGER.warning(
                '[{}] Door link does not exist, hinge joint cannot be created'.format(
                    self._name))
            return False
    ...
    def create_door_from_mesh(self, mesh, set_origin_to_ground=True):
        ...
        if sys.version_info[0] == 2:
            is_string = isinstance(mesh, str) or isinstance(mesh, unicode)
        else:
            is_string = isinstance(mesh, str)
        ...
        if is_string:
            ...
            link_mesh = self._links[self._door_link_name].visuals[0].geometry._mesh
            ...
            if not np.allclose(q[0:3], 0):            
                self._links[self._door_link_name].visuals[0].pose = \
                    self._links[self._door_link_name].visuals[0].pose + Pose(rot=q)
                self._links[self._door_link_name].collisions[0].pose = \
                    self._links[self._door_link_name].collisions[0].pose + Pose(rot=q)
