...
class Walls(SimulationModel):
    ...
    @staticmethod
    def from_random_points(n_points=10, x_min=-10, x_max=10, y_min=-10, 
        y_max=10, height=2.0, thickness=0.1, model_name='walls'):
    ...
    @staticmethod
    def from_random_rectangle(x_center=0, y_center=0, delta_x_min=2, 
        delta_x_max=15, delta_y_min=2, delta_y_max=15, height=2.0, 
        thickness=0.1, model_name='walls'):
        polygon = random_rectangle(x_center, y_center, delta_x_min, 
            delta_x_max, delta_y_min, delta_y_max)
        model = Walls(
            height=height,
            thickness=thickness,
            model_name=model_name,
    ...
    @staticmethod
    def from_random_rectangles(n_rect=5, x_center_min=-10, x_center_max=10, 
        y_center_min=-10, y_center_max=10, delta_x_min=2, delta_x_max=10,
        delta_y_min=2, delta_y_max=10, delete_interiors=False, height=2.0, 
        thickness=0.1, model_name='walls'):
        polygon = random_rectangles(
            n_rect, x_center_min, x_center_max, y_center_min, y_center_max,
            delta_x_min, delta_x_max, delta_y_min, delta_y_max, delete_interiors)
        model = Walls(
            height=height,
            thickness=thickness,
            model_name=model_name,
    ...
    @staticmethod
    def from_random_orthogonal_lines(n_lines=5, x_min=-10, x_max=10, y_min=-10, 
        y_max=10, line_min_length=1, line_max_length=15, height=2.0, 
        thickness=0.1, model_name='walls'):
        lines = random_orthogonal_lines(
            n_lines, x_min, x_max, y_min, y_max, line_min_length, line_max_length)
        model = Walls(
            height=height,
            thickness=thickness,
            model_name=model_name,
    ...
    def add_points(self, points):
        ...
        if not self.is_geometry_duplicate(mp):
    ...
    def create_wall(self, geometry):
        ...
        for poly in wall_polygons:
            ...
            self.add_link(
                ...
                pose=[wall_mesh.centroid[0], wall_mesh.centroid[1], wall_mesh.centroid[2], 0, 0, 0])            
        
    def update_walls(self):
        if self._geometries is None or len(self._geometries) == 0:
            PCG_ROOT_LOGGER.warning(
                'Walls model <{}> has no polygons to describe floorplan'.format(
                    self.name))
            return False
        if not isinstance(self._geometries, list):
            PCG_ROOT_LOGGER.warning('Floor plan polygons must be provided as a list')
            return False
        for geo in self._geometries:
            assert isinstance(geo, (Polygon, MultiPolygon, LineString, MultiLineString)), \
        ...
        for geo in self._geometries:            
            if len(self._floorplan_geometries) == 0:
                ...
                combined_geometry = unary_union(
                    [fg for fg in self._floorplan_geometries if isinstance(fg, (Polygon, MultiPolygon))])
    ...
    def plot_reference_geometries(self, fig=None, ax=None, fig_width=20, fig_height=15, 
        use_matplotlib=True, add_floorplan_reference_geometry_geometries=True, alpha=0.5, 
        line_width=2, line_style='solid', color='tab:blue'):
        for geo in self._geometries:
            fig, ax = plot_shapely_geometry(
                fig=fig,
                ax=ax, 
