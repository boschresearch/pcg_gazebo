...
class Link(object):
    ...
    * `life_timeout` (*type:* `float`, *default:* `None`): Timeout in which
        ...
        if use_approximated_collision:
            if approximated_collision_model == 'box':
                ...
                link.get_collision_by_name('collision').set_cylinder_as_geometry(
                    length=cylinder_kwargs['height'], radius=cylinder_kwargs['radius'])
    ...
    def add_empty_visual(self, name='visual'):
        ...
        * `name` (*type:* `str`, *default:* `visual`): Name of the visual 
        ...
        if self.has_visual(name):
            self._logger.error('Visual with name {} already exists in link {}'.format(
                name, self.name))
            return False
    ...
    def add_empty_collision(self, name='collision'):
        ...
        * `name` (*type:* `str`, *default:* `collision`): Name of the collision 
        ...
        if self.has_collision(name):
            self._logger.error('Collision with name {} already exists in link {}'.format(
                name, self.name))
            return False
        ...
        if sdf.visuals is not None:
            for visual in sdf.visuals:
                if not link.add_visual(Visual.from_sdf(visual)):                    
                    raise AttributeError('Could not import visual element {}'.format(visual.name))
    ...
    def export_to_gazebo_model(self, output_dir, name='model', sdf_version='1.6', 
        version='0.1.0', author_names=None, author_emails=None, description='', 
        ...
        * `generate_sdf_with_version` (*type:* `bool`, *default:* `False`): Parameter description
        ...
        if generate_sdf_with_version:
            output_sdf_file = 'model_{}.sdf'.format(sdf_version.replace('.', '_'))
        else:
        ...
        if not os.path.isdir(output_dir):
            self._logger.error('Output directory does not exist, dir={}'.format(output_dir))
            return False

        output_model_dir = os.path.join(output_dir, name)
        if os.path.isdir(output_model_dir):
            self._logger.error('A model with the same name already exists in output'
    ...
    def add_sensor(self, name, sensor):
        ...
        * `sensor` (*type:* `pcg_gazebo.simulation.sensors.Sensor`): 
        ...
        if name in self._sensors:
            self._logger.error('Sensor with name {} already exists for link {}'.format(
                name, self.name))
            return False
    ...
    def get_footprint(self, mesh_type='collision', pose_offset=None, 
        ...
        * `z_limits` (*type:* `list`, *default:* `None`): Minimum and maximum limits
        ...
        assert mesh_type in ['collision', 'visual'], \
            'Origin of footprints must be either collision or visual geometries'
        if pose_offset is not None:
            assert isinstance(pose_offset, Pose), 'Invalid pose property object'
        else:
            pose_offset = Pose()
