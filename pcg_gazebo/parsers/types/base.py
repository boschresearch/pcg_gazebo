...
class XMLBase(object):
    ...
    def _add_child_element(self, tag, value):
        ...
        if self.has_value():
            ...
            assert creator is not None, 'No creator for {} was found'.format(tag)
            ...
            if issubclass(value.__class__, XMLBase):
                if self._get_child_element_name(tag) != 'empty':
                    if value.has_value():
                        ...
                        assert hasattr(obj, 'value'), 'No element <{}> for <{}>'.format('value', tag)
                        setattr(obj, 'value', value.value)
                    else:
                        assert self._get_child_element_name(tag) == value._NAME, \
                            'Child creator has a different name from input value, received={}, ' \
                            'expected={}'.format(value._NAME, self._get_child_element_name(tag))
                ...
                if isinstance(value, dict):      
                    ...
                    if self._get_child_element_name(tag) != 'empty':
                        ...
                        assert len(value) != 0, 'Dictionary for object {} properties is empty'.format(tag)
                    for elem in value:
                        # Setting attributes, if provided
                        if elem == 'attributes':
                ...
                elif isinstance(value, collections.Iterable) and not obj.has_value():
                            ...
                            for elem in subelem: 
                                ...
                                if elem in obj._CHILDREN_CREATORS:
                                    if 'n_elems' in obj._CHILDREN_CREATORS[elem]:
                                        if obj._CHILDREN_CREATORS[elem]['n_elems'] == '+':
                                            has_mult = True   
                                assert has_mult, 'No multiple elements for {}'.format(elem)
                                obj._add_child_element(elem, subelem[elem])                           
                elif obj.has_value():
                    assert len(obj._CHILDREN_CREATORS) == 0, \
                        'No value parameter found for <{}>, value={}, value type={}'.format(
                            tag, value, type(value))
                    if obj._NAME != 'empty':
                        setattr(obj, 'value', value)
                else:
                    # print('Ignoring element {}, no value found, received={}'.format(tag, value))
                    return

            has_mult = False
            if not self._has_custom_elements:
                assert tag in self._CHILDREN_CREATORS, \
                    '{} element is not a child element from {}'.format(tag, self._NAME)
    ...
    def is_valid(self):
        if not self.has_value():
            if len(self.children) < self._get_num_mandatory_elems():
                PCG_ROOT_LOGGER.error('{} must have at least {} child elements'.format(
                    self._NAME, len(self._CHILDREN_CREATORS)))
                return False
            ...
            for tag in self._CHILDREN_CREATORS:
                ...
                if 'mode' in self._CHILDREN_CREATORS[tag]:
                    is_optional = is_optional or self._CHILDREN_CREATORS[tag]['mode'] != self._mode

                if child_name not in self.children and not is_optional:
                    PCG_ROOT_LOGGER.error('Mandatory element {} not found'.format(child_name))
                    return False

                if child_name in self.children:
                    if not isinstance(self.children[child_name], list):
                        if not isinstance(self.children[child_name], self._get_child_element_creator(tag)):
                            PCG_ROOT_LOGGER.error('Element {} set as the wrong type, expected type={}, actual type={}'.format(
                                child_name, self._get_child_element_creator(tag), type(self.children[child_name])))
                            return False
                    else:
                        for i in range(len(self.children[child_name])):
                            if not isinstance(self.children[child_name][i], self._get_child_element_creator(tag)):
                                PCG_ROOT_LOGGER.error('Element {} #{} set as the wrong type'.format(child_name, i))
                                return False
    ...
    def reset(self, mode=None, with_optional_elements=False):
        if mode is not None and len(self._MODES) > 0:
            assert mode in self._MODES, 'Mode {} does not belong to the list of valid modes={}'.format(mode, self._MODES)
            self._mode = mode
        ...
        if len(self._CHILDREN_CREATORS) > 0:
            ...
            for child in self._CHILDREN_CREATORS:
                if 'optional' in self._CHILDREN_CREATORS[child]:
                    if self._CHILDREN_CREATORS[child]['optional'] and not with_optional_elements:
                        continue
                # Test if element has a mode option and compare to the
                # mode provided
                if self._mode is not None:
                    if 'mode' in self._CHILDREN_CREATORS[child]:
                        if self._mode !=  self._CHILDREN_CREATORS[child]['mode']:
                            continue
                ...
                if 'default' in self._CHILDREN_CREATORS[child]:
                    assert isinstance(self._CHILDREN_CREATORS[child]['default'], list)
                ...
                for child_name in self.children:                    
                    if isinstance(self.children[child_name], list):                        
                        if self._child_exists_in_version(child_name, version):                            
                            ...
                            PCG_ROOT_LOGGER.warning('{} child element not available for version {}'.format(
                                child_name, version))
                    else:
                        if self._child_exists_in_version(child_name, version):
                ...
                if self.children[child] is not None:
                    if not isinstance(self.children[child], list):
                        ...
                        if len(self.children[child].attributes):
                            data[child]['attributes'] = self.children[child].attributes
                    else:
    ...
    def export_xml(self, filename, version='1.6'):
        ...
        content = etree.tostring(xml_root, pretty_print=True, encoding='unicode')
        tree = ElementTree(xml_root)
        with open(filename, 'w+', encoding='utf-8') as output_xml:
            if sys.version_info[0] == 2:
                output_xml.write(unicode('<?xml version="1.0" ?>\n'))
            else:
