...
class TestSimulationObjects(unittest.TestCase):
    def check_joint_names(self, model, joint_names):
        for joint in joint_names:
            self.assertIsNotNone(model.get_joint_by_name(joint), 
    ...
    def check_attributes(self, obj, params):
        for tag in params:
            ...
            if hasattr(att, 'value'):
                ...
                value = att
            self.assertEqual(value, params[tag], 
    ...
    def test_load_test_joint_fixed(self):
        ...
        for tag in PARAMS:
            self.assertEqual(getattr(model, tag), PARAMS[tag], 
                'Element {} should have value {}, retrieved={}'.format(
                    tag, PARAMS[tag], getattr(model, tag)))

        self.assertEqual(model.pose.position.tolist(), [0, 0, 0], 
            'Model position should be [0, 0, 0], retrieved={}'.format(model.pose.position))
        self.assertEqual(model.pose.rpy, [0, 0, 0], 
            'Model orientation should be [0, 0, 0], retrieved={}'.format(model.pose.rpy))     
    ...
    def test_load_test_static_model_object(self):        
        ...
        self.assertEqual(sdf._NAME, 'sdf', 
            'Parsed SDF should be of type SDF, retrieved={}'.format(sdf._NAME))
        ...
        self.assertEqual(len(link.visuals), 1, 'Link should have one visual element')
        ...
        BOX_SIZE = [1, 2, 3]
        self.assertEqual(visual.geometry.get_param('size'), BOX_SIZE, 
            'Visual geometry box should have size {}, retrieved={}'.format(
                BOX_SIZE, visual.geometry.get_param('size')))

        self.assertEqual(len(link.collisions), 1, 'Link should have one collision element')
        ...
        CYL_RADIUS = 2.4
        CYL_LENGTH = 5.6
        self.assertEqual(collision.geometry.get_param('radius'), CYL_RADIUS, 
            'Collision cylinder radius is wrong, retrieved={}, expected={}'.format(
                collision.geometry.get_param('radius'), CYL_RADIUS))
        self.assertEqual(collision.geometry.get_param('length'), CYL_LENGTH, 
            'Collision cylinder length is wrong, retrieved={}, expected={}'.format(
                collision.geometry.get_param('length'), CYL_LENGTH))
        ...
        for tag in PARAMS:
            self.assertEqual(collision.get_ode_friction_param(tag), PARAMS[tag], 
                'Element {} should have value {}, retrieved={}'.format(
        ...
        for tag in PARAMS:
            self.assertEqual(collision.get_ode_contact_param(tag), PARAMS[tag], 
                'Element {} should have value {}, retrieved={}'.format(
                    tag, PARAMS[tag], collision.get_ode_contact_param(tag)))
    ...
    def test_load_test_static_model_sdf(self):        
        ...
        # Parsing visual 
        self.assertEqual(len(sdf.visuals), 1, 'Link should have one visual element') 
        self.assertEqual(sdf.visuals[0].name, 'body_visual', 'Visual must have name body_visual')       
        self.assertIsNotNone(sdf.visuals[0].geometry.box, 
            'Visual has no box element')
        
        BOX_SIZE = [1, 2, 3]
        self.assertEqual(sdf.visuals[0].geometry.box.size.value, BOX_SIZE, 
            'Box has the wrong size, retrieved={}, expected={}'.format(
                sdf.visuals[0].geometry.box.size.value, BOX_SIZE))

        POSE_VISUAL = [0, 0.4, 0, 0, -0.7, 0]        
        self.assertEqual(sdf.visuals[0].pose.value, POSE_VISUAL, 
            'Visual pose is wrong, retrieved={}, expected={}'.format(
                sdf.visuals[0].pose.value, POSE_VISUAL))
        ...
        # Parsing collision
        self.assertEqual(len(sdf.collisions), 1, 'Link should have one collision element')
        ...
        CYL_RADIUS = 2.4
        CYL_LENGTH = 5.6
        self.assertEqual(sdf.collisions[0].geometry.cylinder.radius.value, CYL_RADIUS, 
            'Collision cylinder radius is wrong, retrieved={}, expected={}'.format(
                sdf.collisions[0].geometry.cylinder.radius.value, CYL_RADIUS))
        self.assertEqual(sdf.collisions[0].geometry.cylinder.length.value, CYL_LENGTH, 
            'Collision cylinder length is wrong, retrieved={}, expected={}'.format(
                sdf.collisions[0].geometry.cylinder.length.value, CYL_LENGTH))

        self.assertEqual(sdf.collisions[0].max_contacts.value, 10, 
            'Max. number of contacts is wrong')

        POSE_COLLISION = [0.6, 1.2, 0.05, 0, 0, 0]        
        self.assertEqual(sdf.collisions[0].pose.value, POSE_COLLISION, 
            'Collision pose is wrong, retrieved={}, expected={}'.format(
                sdf.collisions[0].pose.value, POSE_COLLISION))

        MU = 1
        MU2 = 0.5
        self.assertEqual(sdf.collisions[0].surface.friction.ode.mu.value, MU, 
            'Collision mu is wrong, retrieved={}, expected={}'.format(
                sdf.collisions[0].surface.friction.ode.mu.value, MU))
        self.assertEqual(sdf.collisions[0].surface.friction.ode.mu2.value, MU2, 
            'Collision mu2 is wrong, retrieved={}, expected={}'.format(
                sdf.collisions[0].surface.friction.ode.mu2.value, MU2))

        SLIP = 0.5
        self.assertEqual(sdf.collisions[0].surface.friction.torsional.ode.slip.value, SLIP, 
            'Collision torsional slip is wrong, retrieved={}, expected={}'.format(
    ...
    def test_joint(self):
        ...
        for joint_input in valid_inputs:
            ...
            self.assertEqual(joint.name, joint_input['name'], 
                'Joint name was not set, expected={}, retrieved={}'.format(
                    joint_input['name'], joint.name))
            self.assertEqual(joint.parent, joint_input['parent'], 
                'Joint parent name was not set, expected={}, retrieved={}'.format(
                    joint_input['parent'], joint.parent))            
            self.assertEqual(joint.child, joint_input['child'], 
                'Joint child name was not set, expected={}, retrieved={}'.format(
                    joint_input['child'], joint.child))            
            self.assertEqual(joint.type, joint_input['joint_type'], 
                'Joint type was not set, expected={}, retrieved={}'.format(
                    joint_input['joint_type'], joint.type))            
    ...
    def test_sphere_object(self):
        ...
        for s_radius, s_name in zip(valid_radius, names):
            ...
            self.assertEqual(sphere.radius, s_radius, 'Radius should be {}'.format(s_radius))
            self.assertEqual(sphere.name, s_name, 'Name should be {}'.format(s_name))
            ...
            self.assertEqual(sdf.radius.value, s_radius, 'SDF converted radius is invalid')
            
        # Test setting different pose vectors (position + quaternion)
        valid_positions = [[random() for _ in range(3)] for _ in range(N_TESTS)]
        ...
        sdf_types = ['sphere', 'geometry', 'collision', 'visual', 'link', 'model']
        for t in sdf_types:
            sphere = Sphere()
            radius = random()
            name = generate_random_string(5)
            ...
            self.assertEqual(sdf._NAME, t, 'Wrong SDF type in request for {}'.format(t))
            self.assertTrue(sdf.is_valid(), 'Invalid SDF structure was returned')
            ...
            if sdf_radius is not None:
                self.assertEqual(sdf_radius, radius, 'SDF element has the wrong radius,'
    ...
    def test_cylinder_object(self):
        ...
        for cyl_size, cyl_name in zip(valid_sizes, names):
            ...
            cylinder = Cylinder(name=cyl_name, radius=cyl_size[0], length=cyl_size[1])
            self.assertEqual(cylinder.radius, cyl_size[0], 'Radius should be {}'.format(cyl_size[0]))
            self.assertEqual(cylinder.length, cyl_size[1], 'Length should be {}'.format(cyl_size[1]))
            self.assertEqual(cylinder.name, cyl_name, 'Name should be {}'.format(cyl_name))
            ...
            self.assertEqual(sdf.radius.value, cyl_size[0], 'SDF converted radius is invalid')
            self.assertEqual(sdf.length.value, cyl_size[1], 'SDF converted length is invalid')

        # Test setting different pose vectors (position + Euler angles)
        valid_poses = [[random() for _ in range(3)] + [random() * 2 * np.pi for _ in range(3)] for _ in range(N_TESTS)]
        for pose in valid_poses:
            cylinder = Cylinder()
            cylinder.pose = pose
            self.assertEqual(cylinder.pose.position.tolist(), pose[0:3], 
                'Box position should be {}, retrieved={}'.format(pose[0:3], cylinder.pose.position))            

            ref_q = Pose.rpy2quat(*pose[3::])
            q = cylinder.pose.quat
            diff = Pose.get_transform(ref_q, q)

            self.assertAlmostEqual(np.sum(diff[0:3]), 0, 7, 
                'Box orientation element rpy should be {}, retrieved={}'.format(pose[3::], q))
        ...
        for pos, quat in zip(valid_positions, valid_quat):
            ...
            self.assertEqual(cylinder.pose.position.tolist(), pos, 
                'Box position should be {}, retrieved={}'.format(pos, cylinder.pose.position))   

            ref_q = list(quat)
            q = list(cylinder.pose.quat)

            self.assertEqual(ref_q, q, 
                'Quaternion was assigned incorrectly, expected={}, retrieved={}'.format(ref_q, q))
            
        sdf_types = ['cylinder', 'geometry', 'collision', 'visual', 'link', 'model']
        for t in sdf_types:
            cylinder = Cylinder()
            size = [random(), random()]
            name = generate_random_string(5)
            ...
            if sdf_radius is not None and sdf_length is not None:
                self.assertEqual(sdf_radius, size[0], 'SDF element has the wrong radius,'
                    ' expected={}, received={}'.format(size[0], sdf_radius))
                self.assertEqual(sdf_length, size[1], 'SDF element has the wrong length,'
    ...
    def test_box_object(self):
        ...
        for box_size, box_name in zip(valid_sizes, names):
            ...
            self.assertEqual(box.size, box_size, 'Size should be {}'.format(box_size))
            self.assertEqual(box.name, box_name, 'Name should be {}'.format(box_name))
            ...
            sdf = box.to_sdf('box')
            self.assertEqual(sdf.size.value, box_size, 'SDF converted size is invalid')
        ...
        for pose in valid_poses:
            box = Box()
            box.pose = pose
            self.assertEqual(box.pose.position.tolist(), pose[0:3], 
                'Box position should be {}, retrieved={}'.format(pose[0:3], box.pose.position))            
        ...
        for pos, quat in zip(valid_positions, valid_quat):
            ...
            self.assertEqual(box.pose.position.tolist(), pos, 
                'Box position should be {}, retrieved={}'.format(pos, box.pose.position))   
        ...
        for t in sdf_types:
            ...
            if sdf_size is not None:
                self.assertEqual(sdf_size, size, 'SDF element has the wrong size vector,'
