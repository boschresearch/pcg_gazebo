...
class TestURDFParser(unittest.TestCase):
    def test_basic_element_types(self):
        for urdf_tag in URDF_BASIC_OBJ_NAMES:
            ...
            for value, value_str in zip(
                    VALID_BASIC_VALUES[urdf_tag],
                    VALID_OUTPUT_STRING[urdf_tag]):
                obj.value = value
                self.assertEqual(
                    obj.value, value,
    ...
    def test_parse_gazebo_default_link(self):
        ...
        self.assertEqual(
            obj.mu1.value,
            1.0,
            'Wrong value for mu1, received={}, expected={}'.format(
                obj.mu1.value,
                1.0))
        self.assertEqual(
            obj.mu2.value,
            1.0,
            'Wrong value for mu2, received={}, expected={}'.format(
                obj.mu2.value,
                1.0))
        self.assertEqual(
            obj.kp.value,
            1000000.0,
            'Wrong value for kp, received={}, expected={}'.format(
                obj.kp.value,
                1000000.0))
        self.assertEqual(
            obj.kd.value,
            100.0,
            'Wrong value for kd, received={}, expected={}'.format(
                obj.kd.value,
                100.0))
        self.assertEqual(
            obj.minDepth.value,
            0.001,
            'Wrong value for minDepth, received={}, expected={}'.format(
                obj.minDepth.value,
                0.001))
        self.assertEqual(
            obj.maxVel.value,
            1.0,
            'Wrong value for maxVel, received={}, expected={}'.format(
                obj.maxVel.value,
                1.0))
        self.assertEqual(
            obj.selfCollide.value,
            False,
            'Wrong value for selfCollide, received={}, expected={}'.format(
                obj.selfCollide.value,
                False))
        self.assertEqual(
            obj.maxContacts.value,
            15,
            'Wrong value for maxContacts, received={}, expected={}'.format(
                obj.maxContacts.value,
                15))
    ...
    def test_parse_gazebo_default_joint(self):
        ...
        self.assertEqual(
            obj.stopCfm.value,
            0.1,
            'Wrong value for stopCfm, received={}, expected={}'.format(
                obj.stopCfm.value,
                0.1))
        self.assertEqual(
            obj.stopErp.value,
            0.3,
            'Wrong value for stopErp, received={}, expected={}'.format(
                obj.stopErp.value,
                0.3))
